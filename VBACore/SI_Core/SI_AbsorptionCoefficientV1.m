function [AlphaKSitu, AlphakLab, AlphAvg, Kijs] = SI_AbsorptionCoefficientV1(Elements, Geo)
%
...
for perimeter = 1:4
    ...
    for j = 1:ColumKIJ
        if strcmp(Geo, 'GeoUnity34') || strcmp(Geo, 'Geo34')
            %             Perimt(j) = sqrt(fcj(perimeter,j)./fref).*(10.^(-KIJ(1,j)/10));
            %         else
            Perimt(j) = sqrt(fcj{perimeter, 1}(j)./fref) .* (10.^(-KIJ(j) / 10));
            Kijs{perimeter} = Kij;
        end
        if strcmp(Geo, 'GeometriesFlat') || strcmp(Geo, 'Geometries')
            %             Perimt(j) = sqrt(fcj(perimeter,j)./fref).*(10.^(-KIJ(1,j)/10));
            %         else
            Perimt(j) = sqrt(fcj{perimeter, 1}(j)./fref) .* (10.^(-KIJ(j) / 10));
            Kijs{perimeter} = Kij;
