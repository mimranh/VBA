...
EtaInitial = Elements.InternalLossFactor;
Material = Elements.Type;
S = Elements.Area;
fc = Elements.fc;
SigmaFeeWave = Elements.SigmaFW;
Lk = Elements.JunctionLength';
AlphaKSitu = Elements.AlphaKSitu';
AlphaKLab = Elements.AlphakLab';
...
Factor = sum(Lk.*AlphaKSitu);
EtaTotalSitu = EtaInitial + ((2 * Rho0 * c0 .* SigmaFeeWave) ./ (2 * pi * f * ElementMass)) + (c0 ./ ((pi.^2) * S .* sqrt(f.*fc))) .* Factor;
EtaElementRoom = ((2 * Rho0 * c0 .* SigmaFeeWave) ./ (2 * pi * f * ElementMass));
EtaJunction = (c0 ./ ((pi.^2) * S .* sqrt(f.*fc))) .* Lk .* AlphaKSitu;
% EtaTotal = EtaTotalG;
